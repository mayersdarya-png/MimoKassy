...
- **Справочные пункты** — правила студии, контакты поддержки, версия приложения.
...
| **Контекст** | Чаще всего — запись на ходу, с телефона, одной рукой. Заход в «Профиль» обычно эпизодический: свериться с данными или выйти на чужом/общем устройстве. |
...
**Классы** ([SCR-002](SCR-002-slot-list.md)) · **Мои записи** ([SCR-005](SCR-005-my-bookings.md)) ·
...
| Справочные пункты | «Правила студии», «Поддержка», «Версия приложения» | конфиг / статика | read; правила/поддержка — переходы, версия — текст |
...
3. **Справочные пункты** — «Правила студии» ›, «Поддержка» ›, «Версия приложения».
...
- **Обязательное подтверждение** перед выходом — защита от случайного нажатия. Подтверждение
  оформляется как диалог/шторка по правилам [foundations §4.3](00-foundations.md#43-bottom-sheet-шторки-bs-001--bs-002--bs-003):
  два явных действия — «Выйти» и «Отмена»; закрытие шторки = отмена.
...
- **Правила студии** и **Поддержка** — строки-переходы (›) на статические/внешние ресурсы (адрес/ссылка — параметр конфигурации).
...
| Пояснение | «Чтобы снова записаться на класс, нужно будет войти по номеру телефона.» |
...
- **Крупная, читаемая контактная информация** — имя и телефон контрастны и комфортны для чтения.
...
3. **Справочные пункты — да.** Правила студии, поддержка, версия приложения (§6.4); адреса/ссылки — параметр конфигурации.
...
| **FR-47, FR-48, FR-49** | Просмотр/редактирование профиля, удаление аккаунта, выход. |
